...
class Queue {           // 佇列資料結構
    public:
        ...
        JobType GetFront() { // 取得front的資料
            return front->Data ;
        } // GetFront()

        void Enqueue( JobType input ) { // 新增資料至front
            if ( IsEmpty() ) {
                front = new List ;
                front->Data = input ;
                front->next = NULL ;
...
class AnsList {     // 用來儲存完成及被取消的資料
    ...
    public:
        ...
        void addAbortJob( int OID, int abort, int delay ) { // 加入至abort清單
            abortType tmp;
            tmp.OID = OID;
            tmp.abort = abort;
            tmp.delay = delay;
        ...
        void addDoneJob( int OID, int departure, int delay ) {  // 加入Done至清單
            doneType tmp;
            tmp.OID = OID;
        ...
        void putAll( string fileID ) {  // 將完成處理之清單進行寫檔
