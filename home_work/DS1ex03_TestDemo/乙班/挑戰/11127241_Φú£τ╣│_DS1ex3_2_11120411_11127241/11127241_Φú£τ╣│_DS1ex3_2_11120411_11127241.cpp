...
class ReadFile      
{
    ...
    //任務三的輸出.txt
    void CloseFileMisson_3(vector<vector<int>> abortjobsdata, vector<vector<int>> donejobsdata, double successrate_value, double averagedelay_value, int resent_file ){
        
        string filename = "double" + to_string(resent_file) + ".txt";
        outfile.open(filename);
...
class Missions{
    ...
    void mission_2(){      
        ...
        list.CloseFileMisson_2( abortjobsdata, donejobsdata, successrate( abortjobsdata, donejobsdata ), averagedelay_2( abortjobsdata, donejobsdata ), resent_file ) ;
        cout << "The simulation is running..." << endl ;
        cout << "See output" << resent_file << ".txt" << endl ;
    }

    void mission_3(){      //__________________________________任務三
        ReadFile list;
        vector<vector<int>> ToDoList;
        ...
        vector<vector<int>> abortjobsdata ;  //  abort jobs的資料
        vector<vector<int>> donejobsdata ;  //  done jobs的資料
        abortjobsdata.resize(0, vector<int>(4, 0));  //  先設定行數是3
        donejobsdata.resize(0, vector<int>(4, 0));  //  先設定行數是3
        process_3( abortjobsdata, donejobsdata, ToDoList ) ;
        list.CloseFileMisson_3( abortjobsdata, donejobsdata, successrate( abortjobsdata, donejobsdata ), averagedelay_3( abortjobsdata, donejobsdata ), resent_file ) ;
        cout << "The simulation is running..." << endl ;
        cout << "See double" << resent_file << ".txt" << endl ;
    }
        ...
        while ( num != sorteddata.size() || !cpuisEmpty(cpuArray) || !Queue.isEmpty() ) {  //  結束條件確保跑完sort和cpu和queue
            ...
            if ( !cpuisEmpty(cpuArray) ) {  //cpu裡面有東西
                if ( time == cpuArray[0].Duration + starttime )  { //cpu跑完了
                    cpufinish_2( cpuArray, donejobsdata, time ) ;
                    cpuArray.clear() ;
                    if ( !Queue.isEmpty() ) {  //如果有queue先跑queue裡面的    
                        while ( !Queue.isEmpty() && queueisok != false ) {
                            ...
                            if ( time < currentQueueJob.TimeOut ) {  //當前時間在此逾時之前
                                ...
                                situation_42_2( currentQueueJob, abortjobsdata, time ) ;
                ...
                if ( !cpuisEmpty(cpuArray) ) {
                    if ( time >= cpuArray[0].TimeOut ) { //此為4.3的情況
                        situation_43_2( cpuArray, abortjobsdata, time ) ;
                        cpuArray.clear() ;
                        ...
                        if ( !Queue.isEmpty() ) {  //如果有queue先跑queue裡面的     
                            while ( !Queue.isEmpty() && queueisok != false ) {
                                ...
                                if ( time < currentQueueJob.TimeOut ) {  //當前時間在此逾時之前
                                    ...
                                    situation_42_2( currentQueueJob, abortjobsdata, time ) ; //  此為4.2的情況
                                    Queue.deQueue() ;  
                                }
            ...
            while ( num != sorteddata.size() && time == sorteddata[num][1] ) {  //跑到arrival的時間
                if ( cpuisEmpty(cpuArray) ) { //  cpu是否為空
                    ...
                    if ( !Queue.isFull() ) {  //沒滿把下一個加進queue裡面
                        ...
                        situation_41_2(sorteddata, abortjobsdata, num );  //此為4.1的情況
                    }
                }
    ...
    //任務三的過程
    void process_3(vector<vector<int>> &abortjobsdata, vector<vector<int>> &donejobsdata, vector<vector<int>> &sorteddata) {
        int num = 0 ;  //  計數器
        int time = 1 ;  //現在時間
        int starttime_1 = 0 ; //cpu1開始時間
        int starttime_2 = 0 ; //cpu2開始時間
        bool firsttimecpu_1 = true ;  //是否為第一次跑cpu1
        bool firsttimecpu_2 = true ;  //是否為第一次跑cpu2
        queue Queue_1 ; //  把queue帶進來
        queue Queue_2 ;
        vector<Sortedjobs> cpuArray_1;
        vector<Sortedjobs> cpuArray_2;
        while ( num != sorteddata.size() || !cpuisEmpty(cpuArray_1) || !cpuisEmpty(cpuArray_2) || !Queue_1.isEmpty() || !Queue_2.isEmpty() ) {  //  結束條件確保跑完sort和cpu和queue
            bool queueisok_1 = true ;
            bool queueisok_2 = true ;
            if ( !cpuisEmpty(cpuArray_1) || !cpuisEmpty(cpuArray_2) ) {  //cpu裡面有東西
                //先找哪個cpu是在跑的
                if ( whichrun( cpuArray_1, cpuArray_2 ) == 1 ) {  //  cpu1有東西
                    if ( time == cpuArray_1[0].Duration + starttime_1 )  { //cpu跑完了
                        cpufinish_3( cpuArray_1, donejobsdata, time, 1 ) ;
                        cpuArray_1.clear() ;
                        if ( !Queue_1.isEmpty() ) {  //如果有queue先跑queue裡面的    
                            while ( !Queue_1.isEmpty() && queueisok_1 != false ) {
                                Sortedjobs currentQueueJob = Queue_1.getFront();
                                if ( time < currentQueueJob.TimeOut ) {  //當前時間在此逾時之前
                                    cpuReplenish_2( currentQueueJob, cpuArray_1 ) ;
                                    Queue_1.deQueue() ;
                                    starttime_1 = time ;  //紀錄cpu開始跑的時間
                                    queueisok_1 = false ;  
                                }
                                else {  //當前時間在此逾時之後
                                    situation_42_3( currentQueueJob, abortjobsdata, time, 1 ) ;
                                    Queue_1.deQueue() ;  //此為4.2的情況
                                }
                            } 
                    ...
                    if ( !cpuisEmpty(cpuArray_1) ) {
                        if ( time >= cpuArray_1[0].TimeOut ) { //此為4.3的情況
                            situation_43_3( cpuArray_1, abortjobsdata, time, 1 ) ;
                            cpuArray_1.clear() ;
                            ...
                            if ( !Queue_1.isEmpty() ) {  //如果有queue先跑queue裡面的     
                                while ( !Queue_1.isEmpty() && queueisok_1 != false ) {
                                    ...
                                    if ( time < currentQueueJob.TimeOut ) {  //當前時間在此逾時之前
                                        ...
                                        situation_42_3( currentQueueJob, abortjobsdata, time, 1 ) ; //  此為4.2的情況
                                        Queue_1.deQueue() ;  
                                    }
                ...
                else if ( whichrun( cpuArray_1, cpuArray_2 ) == 2 ) {  //  cpu2有東西
                    if ( time == cpuArray_2[0].Duration + starttime_2 )  { //cpu跑完了
                        cpufinish_3( cpuArray_2, donejobsdata, time, 2 ) ;
                        cpuArray_2.clear() ;
                        if ( !Queue_2.isEmpty() ) {  //如果有queue先跑queue裡面的    
                            while ( !Queue_2.isEmpty() && queueisok_2 != false ) {
                                Sortedjobs currentQueueJob = Queue_2.getFront();
                                if ( time < currentQueueJob.TimeOut ) {  //當前時間在此逾時之前
                                    cpuReplenish_2( currentQueueJob, cpuArray_2 ) ;
                                    Queue_2.deQueue() ;
                                    starttime_2 = time ;  //紀錄cpu開始跑的時間
                                    queueisok_2 = false ;  
                                }
                                else {  //當前時間在此逾時之後
                                    situation_42_3( currentQueueJob, abortjobsdata, time, 2 ) ;
                                    Queue_2.deQueue() ;  //此為4.2的情況
                                }
                            } 
                    ...
                    if ( !cpuisEmpty(cpuArray_2) ) {
                        if ( time >= cpuArray_2[0].TimeOut ) { //此為4.3的情況
                            situation_43_3( cpuArray_2, abortjobsdata, time, 2 ) ;
                            cpuArray_2.clear() ;
                            ...
                            if ( !Queue_2.isEmpty() ) {  //如果有queue先跑queue裡面的     
                                while ( !Queue_2.isEmpty() && queueisok_2 != false ) {
                                    ...
                                    if ( time < currentQueueJob.TimeOut ) {  //當前時間在此逾時之前
                                        ...
                                        situation_42_3( currentQueueJob, abortjobsdata, time, 2 ) ; //  此為4.2的情況
                                        Queue_2.deQueue() ;  
                                    }
                ...
                else if ( whichrun( cpuArray_1, cpuArray_2 ) == 3 ) {  //  cpu1,2有東西
                    if ( time == cpuArray_1[0].Duration + starttime_1 )  { //cpu跑完了
                        cpufinish_3( cpuArray_1, donejobsdata, time, 1 ) ;
                        cpuArray_1.clear() ;
                        if ( !Queue_1.isEmpty() ) {  //如果有queue先跑queue裡面的    
            ...
            while ( num != sorteddata.size() && time == sorteddata[num][1] ) {  //跑到arrival的時間
                if ( cpuisEmpty(cpuArray_1) || cpuisEmpty(cpuArray_2) ) { //  cpu是否有空位
                    if ( whichrun( cpuArray_1, cpuArray_2 ) == 2 || whichrun( cpuArray_1, cpuArray_2 ) == 0 ) {  //cpu2裡面有東西,cpu1沒東西把目前資料放入cpu1
                        if ( firsttimecpu_1 == true ) {  //第一次跑cpu
                            cpuReplenish_1( sorteddata, cpuArray_1, num ) ;
                            starttime_1 = time ;  //紀錄cpu開始跑的時間
                            firsttimecpu_1 = false ;
                        }
                        else {  //不是第一次跑
                            if ( !Queue_1.isEmpty() ) {  //如果有queue先跑queue裡面的
                    ...
                    else if ( whichrun( cpuArray_1, cpuArray_2 ) == 1 ) {   //cpu1裡面有東西,cpu2沒東西把目前資料放入cpu2
                        if ( firsttimecpu_2 == true ) {  //第一次跑cpu
                            cpuReplenish_1( sorteddata, cpuArray_2, num ) ;
                            starttime_2 = time ;  //紀錄cpu開始跑的時間
                            firsttimecpu_2 = false ;
                        }
                        else {  //不是第一次跑
                            if ( !Queue_2.isEmpty() ) {  //如果有queue先跑queue裡面的
                    ...
                    //queue有分
                    //queue_1.size>queue_2.size     1>2         放進queue_2
                    //queue_1.size=queue_2.size     1=2且沒滿   放進queue_1
                    //queue_1.size<queue_2.size     1<2         放進queue_1
                    //queue_1.size=queue_2.size=3   都滿了      4.1丟到abort

                    if ( Queue_1.size() > Queue_2.size() ) {        //1>2 放進queue_2
                        queueReplenish ( sorteddata, Queue_2, num ) ;
                    }
                    else if ( Queue_1.size() <= Queue_2.size() && !Queue_1.isFull() ) {  //1=2 且queue1沒滿,放進queue_1
                        queueReplenish ( sorteddata, Queue_1, num ) ;
                    }
                    else {
                        situation_41_3(sorteddata, abortjobsdata, num, 0 );  //此為4.1的情況
                    }
    ...
    //找事哪個cpu再跑,1跑回傳1,2跑回傳2,都跑回傳3
    int whichrun ( vector<Sortedjobs> cpuArray_1, vector<Sortedjobs> cpuArray_2 ) {
        if ( !cpuisEmpty(cpuArray_1) && !cpuisEmpty(cpuArray_2) ) {     //cpu1,2有東西
            return 3 ;
        }
        else if ( !cpuisEmpty(cpuArray_1) ) {   //cpu1有東西
            return 1 ;
        }
        else if ( !cpuisEmpty(cpuArray_2) ) {   //cpu2有東西
            return 2 ;
        }
    ...
    //計算average delay----任務二
    double averagedelay_2 ( vector<vector<int>> &abortjobsdata, vector<vector<int>> &donejobsdata) {
        double total = 0 ;
    ...
    //計算average delay----任務三
    double averagedelay_3 ( vector<vector<int>> &abortjobsdata, vector<vector<int>> &donejobsdata) {
        double total = 0 ;
        for (int i = 0; i < donejobsdata.size(); i++)
        {
            total = total + donejobsdata[i][3] ;
        }
        
        for (int i = 0; i < abortjobsdata.size(); i++)
        {
            total = total + abortjobsdata[i][3] ;
        }
    ...
    //cpu完成要做的事----任務二
    void cpufinish_2 ( vector<Sortedjobs> cpuArray, vector<vector<int>> &donejobsdata, int time) {
        vector<int> doneJobRow = { cpuArray[0].OID, time, time - cpuArray[0].Arrival - cpuArray[0].Duration} ;
        donejobsdata.push_back(doneJobRow);
    }

    //cpu完成要做的事----任務三
    void cpufinish_3 ( vector<Sortedjobs> cpuArray, vector<vector<int>> &donejobsdata, int time, int CID) {
        vector<int> doneJobRow = { cpuArray[0].OID, CID, time, time - cpuArray[0].Arrival - cpuArray[0].Duration} ;
        donejobsdata.push_back(doneJobRow);
    }

    //找出最大的timeout----任務二
    int biggesttime_2 ( vector<vector<int>> &sorteddata) {
        int n = 0 ;
        int thebiggest = 0 ;
    ...
    //4.1 佇列已滿的情況----任務二
    void situation_41_2 ( vector<vector<int>> &sorteddata, vector<vector<int>> &abortjobsdata, int num ) { 
        vector<int> abortJobRow = {sorteddata[num][0], sorteddata[num][1], 0};
        abortjobsdata.push_back(abortJobRow);
    }

    //4.1 佇列已滿的情況----任務三
    void situation_41_3 ( vector<vector<int>> &sorteddata, vector<vector<int>> &abortjobsdata, int num, int CID ) { 
        vector<int> abortJobRow = {sorteddata[num][0], CID, sorteddata[num][1], 0};
        abortjobsdata.push_back(abortJobRow);
    }

    //4.2 佇列取出發現逾時----任務二
    void situation_42_2 ( Sortedjobs currentQueueJob, vector<vector<int>> &abortjobsdata, int time) {
        vector<int> abortJobRow = { currentQueueJob.OID, time, time - currentQueueJob.Arrival };  //{OID, 取出時刻, abort - arrival}
        abortjobsdata.push_back(abortJobRow);
    }

    //4.2 佇列取出發現逾時----任務三
    void situation_42_3 ( Sortedjobs currentQueueJob, vector<vector<int>> &abortjobsdata, int time, int CID) {
        vector<int> abortJobRow = { currentQueueJob.OID, CID, time, time - currentQueueJob.Arrival };  //{OID, CID, 取出時刻, abort - arrival}
        abortjobsdata.push_back(abortJobRow);
    }

    //4.3 cpu執行時逾時----任務二
    void situation_43_2 ( vector<Sortedjobs> cpuArray, vector<vector<int>> &abortjobsdata, int time) {
    ...
    //4.3 cpu執行時逾時----任務三
    void situation_43_3 ( vector<Sortedjobs> cpuArray, vector<vector<int>> &abortjobsdata, int time, int CID) {
        //{OID, CID, timeout, abort - arrival}
        vector<int> abortJobRow = { cpuArray[0].OID, CID, time , time - cpuArray[0].Arrival };
        abortjobsdata.push_back(abortJobRow);
    }

    //超過biggesttime要清空queue並把他放到abort----任務二
    void situation_overtime_queue_2 ( Sortedjobs currentQueueJob, vector<vector<int>> &abortjobsdata, int time) {
        vector<int> abortJobRow = { currentQueueJob.OID, time, time - currentQueueJob.Arrival };  //{OID, 取出時刻, abort - arrival}
        abortjobsdata.push_back(abortJobRow);
    }

    //超過biggesttime要清空cpu並把他放到abort----任務二
    void situation_overtime_cpu_2 ( vector<Sortedjobs> cpuArray, vector<vector<int>> &abortjobsdata, int time) {
        //{OID, timeout, abort - arrival}
        vector<int> abortJobRow = { cpuArray[0].OID, time , time - cpuArray[0].Arrival };
        abortjobsdata.push_back(abortJobRow);
    }
