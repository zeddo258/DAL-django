...
public:
	...
	void computeStat(){
		...
		for( int i = 0; i < abortlist.size(); i++ )
			avg += abortlist[i].Delay;
		
		for( int i = 0; i < donelist.size(); i++ )
			avg += donelist[i].Delay;
		
		float a = abortlist.size() + donelist.size();
		avg = avg / a;	// 計算平均Delay時間 
		avg = round( avg * 100.0) / 100.0;
		
		float b = donelist.size();
		successRate = b / a*100;	// 計算成功率 
	...
	void putall( string fileName ){
		...
		if ( !outFile.is_open() ){
			...
			outFile << "\t[Aborts Jobs]" << endl << "\tOID\tAbort\tDelay" << endl;	// 將取消清單存入output???.txt 
			for( int i = 0; i < abortlist.size(); i++ ){ 	
				outFile <<"["<<i+1<<"]"<< "\t"<< abortlist[i].OID << "\t" << abortlist[i].Abort << "\t" << abortlist[i].Delay << "\t" << endl;
			} // for
			 
			outFile << "\t[Jobs Done]" << endl << "\tOID\tDeparture\tDelay" << endl;	// 將完成清單存入output???.txt 
			for( int i = 0; i < donelist.size(); i++ ){ 
...
/*	void finishQ(){
	...
	dataname supcpu( int time ) {
    	...
    	while (!allQ.isEmpty()) {
        	...
        	else if ( temp.TimeOut == time && temp.Duration == 0 ) {
        		return temp;
			}
		...
		while( time <= biggest() ){
			...
			if ( pretime < cur.Arrival ){
				...
				while ( !allQ.isEmpty() ) {
					dataname temp;
					allQ.getFront( temp );
					answers.addAbortJob( temp.OID, temp.TimeOut, temp.Arrival );
					allQ.dequeue();
				}
			//	allQ.clearQ();
			} 
			
			while( time == pretime + cur.Duration && cur.OID != 0 ){	// 目前處理的資料時間到了要取出
			...
			// cpu執行逾時就丟進abort並跟新pretime 
			if ( pretime + cur.Duration > cur.TimeOut && cur.OID != 0 ){
				if ( cur.TimeOut <= pretime ) {
					answers.addAbortJob( cur.OID, pretime, pretime-cur.Arrival );
					cur = supcpu( time );	
				}
				else if ( pretime < cur.TimeOut ){
					answers.addAbortJob( cur.OID, cur.TimeOut, cur.TimeOut-cur.Arrival );
					pretime = cur.TimeOut;
					cur = supcpu( cur.TimeOut );
				}
			} // if
