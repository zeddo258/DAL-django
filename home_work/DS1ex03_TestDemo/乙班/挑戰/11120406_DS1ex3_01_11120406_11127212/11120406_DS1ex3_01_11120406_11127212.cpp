...
class JobQueue{
	private :
		...
		QueueNode *frontPtr = NULL;	// 指向佇列的前端 
		QueueNode *tailPtr = NULL ;	// 指向佇列的後端 
...
class AutoSimulate {
	...
	void delQ(jobType canceljob, int CID) {	// 因為逾時而要取消的資料								// declaration : delete an old job from a queue
	...
	void Qtoprocess(int N) {				// 從佇列取出並開始執行 
		jobType getjob ;
		queue[N].isprocess = false ;		
		while (!queue[N].allQ.isEmpty()) {	// 佇列不為空才會從佇列取出執行 
		...
		if (queue[N].allQ.isEmpty() && !queue[N].isprocess) { // 如果沒有工作可執行，佇列也是空的，減去1秒的時間差 

		needstop = true ;}
	}
	void success(jobType job,int CID) {				// 執行成功的job放入 Done 的 List 儲存 
		int delay ;
		delay = nowtime - job.duration - job.arrival ;
		answers.addDoneJob(job.OID,CID, nowtime, delay) ;
	} // execute()
	void timeout(jobType timeout, int CID) {	// 處理執行到一半而逾時的job
	...
	public :
		...
		void excute() {		// the main excute of simulation
			...
			while(true) {	// run simulation
				...
				if ( nowinfor == jobsize && !haveJob() && isAllempty() ) break ;	// 如果全部跑完，停止simulation 的運行 
				if (!isAllprocess() && !isfinish() && nowtime <= headJob.arrival) {	// 佇列為空和沒有job在運行時，直接跳到下一個有工作進來的arrival 
...
int main () { 
	...
	do {
		...
		cin >> command ;
		if ( command == "0" ) { cout << endl << "程式結束" << endl ; break ; }
