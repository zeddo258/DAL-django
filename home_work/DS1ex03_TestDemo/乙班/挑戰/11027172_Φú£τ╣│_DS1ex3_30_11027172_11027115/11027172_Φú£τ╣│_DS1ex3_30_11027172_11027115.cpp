// DS1ex3_30_11027172³¯´ä¥Í_11027115¤ý·q»¨
#include <iostream>
#include <string>
#include <vector>
#include <tuple>
...
void shell_sort(std::vector<std::tuple<std::string, std::string, std::string, std::string>> &Order){
                                        //  0:OID  1:Arrival  2:Duration  3:Deadline
...
void Two_CPU(){std::vector<std::tuple<std::string, std::string, std::string, std::string>> Order;
    std::string filename;
    std::string word;
    std::ifstream Read;
    std::ofstream Write;
    ...
    int order_count = 0;
    bool work1_empty = true;
    bool work2_empty = true;
    Queue<std::tuple<std::string, std::string, std::string, std::string>> waiting1, waiting2;
    //               0.OID        1.Arrival    2.Duration   3.TimeOut
    std::tuple<std::string, std::string, std::string, std::string, int >  working1, working2;
    //         0.OID        1.Arrival     2.Duration  3.TimeOut    4.StartWorking    
    std::vector<std::tuple<std::string, std::string, std::string, std::string>> abort_list;
    //                     0.OID        1.CID        2.Abort      3.Delay
    std::vector<std::tuple<std::string, std::string, std::string, std::string>> JobDone_list;
    //                     0.OID        1.CID        2.Departure      3.Delay
    //* ------------ SIMULATION ------------
    for (int now = 0; order_count < Order.size() || !waiting1.empty() || !work1_empty || !work2_empty ; now++) {
        //!  check process which is working is finished
        if( !work1_empty && std::get<4>(working1) + std::stoi(std::get<2>(working1)) == now ){
            JobDone_list.push_back(std::make_tuple(std::get<0>(working1), "1", \
                std::to_string(now), std::to_string(now - std::stoi(std::get<1>(working1)) - std::stoi(std::get<2>(working1)))));
            work1_empty = true;
        }

        if( !work2_empty && std::get<4>(working2) + std::stoi(std::get<2>(working2)) == now ){
            JobDone_list.push_back(std::make_tuple(std::get<0>(working2), "2", \
                std::to_string(now), std::to_string(now - std::stoi(std::get<1>(working2)) - std::stoi(std::get<2>(working2)))));
            work2_empty = true;
        }

        if (!work1_empty && now == std::stoi(std::get<3>(working1))){  // Over time
            abort_list.push_back(std::make_tuple(std::get<0>(working1), "1", \
            std::to_string(now), std::to_string(now - std::stoi(std::get<1>(working1)))));
            work1_empty = true;
        }
        
        if (!work2_empty && now == std::stoi(std::get<3>(working2))){  // Over time
            abort_list.push_back(std::make_tuple(std::get<0>(working2), "2", \
            std::to_string(now), std::to_string(now - std::stoi(std::get<1>(working2)))));
            work2_empty = true;
        }

        //! add process into work space 
        if (work1_empty) {  // cpu1 is not working now
            while(!waiting1.empty() && work1_empty){   //* queue is not empty
                if(std::stoi(std::get<3>(waiting1.front())) > now){  //? on time
                    working1 = std::make_tuple(std::get<0>(waiting1.front()), std::get<1>(waiting1.front()),\
                                    std::get<2>(waiting1.front()), std::get<3>(waiting1.front()), now);
                    waiting1.pop();
                    work1_empty = false;
                    if (now >= std::stoi(std::get<3>(working1))){  // Over time
                        abort_list.push_back(std::make_tuple(std::get<0>(working1), "1", \
                            std::get<3>(working1), std::to_string(now - std::stoi(std::get<1>(working1)))));
                    }
                }
                else{   //? over time
                    abort_list.push_back(std::make_tuple(std::get<0>(waiting1.front()),"1",\
                         std::get<3>(waiting1.front()), std::to_string(now - std::stoi(std::get<1>(waiting1.front())))));
                    waiting1.pop();
                }
            }
            if(work1_empty && order_count < Order.size() && now == std::stoi(std::get<1>(Order[order_count]))){
                //* take the task from Order directly
                working1 = std::make_tuple(std::get<0>(Order[order_count]),std::get<1>(Order[order_count])\
                                    ,std::get<2>(Order[order_count]),std::get<3>(Order[order_count]), now);
                order_count++;
                work1_empty = false;
            }
        ...
        if (work2_empty) {  // cpu2 is not working now
            while(!waiting2.empty() && work2_empty){   //* queue is not empty
                if(std::stoi(std::get<3>(waiting2.front())) > now){  //? on time
                    working2 = std::make_tuple(std::get<0>(waiting2.front()), std::get<1>(waiting2.front()),\
                                    std::get<2>(waiting2.front()), std::get<3>(waiting2.front()), now);
                    waiting2.pop();
                    work2_empty = false;
                    if (now >= std::stoi(std::get<3>(working2))){  // Over time
                        abort_list.push_back(std::make_tuple(std::get<0>(working2), "2", \
                            std::get<3>(working2), std::to_string(now - std::stoi(std::get<1>(working2)))));
                    }
                }
                else{   //? over time
                    abort_list.push_back(std::make_tuple(std::get<0>(waiting2.front()),"2",\
                        std::get<3>(waiting2.front()), std::to_string(now - std::stoi(std::get<1>(waiting2.front())))));
                    waiting2.pop();
                }
            }
            if(work2_empty && order_count < Order.size() && now == std::stoi(std::get<1>(Order[order_count]))){
                //* take the task from Order directly
                working2 = std::make_tuple(std::get<0>(Order[order_count]),std::get<1>(Order[order_count])\
                                    ,std::get<2>(Order[order_count]),std::get<3>(Order[order_count]), now);
                order_count++;
                work2_empty = false;
            }
        ...
        while (order_count < Order.size() && now == std::stoi(std::get<1>(Order[order_count]))) {
            // cpu is working now, there have other order and order is arriving
            if(waiting1.Size() == 3 && waiting2.Size() == 3)
                abort_list.push_back(std::make_tuple(std::get<0>(Order[order_count]),"0", std::get<1>(Order[order_count]), "0"));
            else if (waiting1.Size() > waiting2.Size())
                waiting2.push(Order[order_count]);
            else
                waiting1.push(Order[order_count]);
    ...
    for(int i = 0 ; i < abort_list.size() ; i++){
        Write << "[" << i + 1 << "]\t" \
            << std::get<0>(abort_list[i]) << "\t" << std::get<1>(abort_list[i]) << "\t" << std::get<2>(abort_list[i]) << "\t" << std::get<3>(abort_list[i]) << endl;
        delay_count += std::stoi(std::get<3>(abort_list[i]));
    }
    Write <<"\t[Jod Done]\n\tOID\tCID\tDeparture\tDelay\n";
    for (int i = 0; i < JobDone_list.size(); i++) {
        Write << "[" << i + 1 << "]\t" \
            << std::get<0>(JobDone_list[i]) << "\t" << std::get<1>(JobDone_list[i]) << "\t" << std::get<2>(JobDone_list[i]) << "\t" << std::get<3>(JobDone_list[i]) << endl;
        delay_count += std::stoi(std::get<3>(JobDone_list[i]));
    }
    // aberage delay time output as e.g. 0.00 ms
    Write << "[Average Delay]\t" << std::fixed << std::setprecision(2) << round((float)(delay_count) / (float)Order.size()* 100)/100 << " ms" << endl;
    Write << "[Success Rate]\t" << std::fixed << std::setprecision(2) << round((float)(JobDone_list.size()) / (float)Order.size() * 10000)/100  <<" %";
