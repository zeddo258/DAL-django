...
class mission2 {
    public:
        ...
        int quesize1 = 0; // 佇列已有幾個
        int cpubegin = 0; // 該cpu開始時間
        int cpubegin1 = 0; // 該cpu開始時間
        int whichdata = 0; // 第幾筆訂單
        float donenums = 0; // 訂單完成數量
        float totaldelay = 0; // 總延誤

        void logdata ( string x, int cpunum ); // 記錄done或abort
		...
		if ( cmd == "1" ) {
                ...
                while (  m2.finish == false   ) {
                    ...
                    if ( m2.process == true ) {
                        if ( m2.nowtime-m2.cpubegin == cpu[0].duration ) { // 該訂單處理完成
                            ...
                            m2.logdata( "done", 1 ); // 記錄至donelist
                            cpu.pop_back();
                            m2.process = false;
                            m2.newjob( 1 ); // 從佇列抓取新工作

                        }
                        else if ( m2.nowtime == cpu[0].timeOut ) { // 該訂單逾時
                            m2.logdata( "abort", 1 ); // 記錄至adortlist
                            cpu.pop_back();
                            m2.process = false;
                            m2.newjob( 1 ); // 從佇列抓取新工作
            ...
            if( ReadTxt() ) {
                ...
                while (  m2.finish == false   ) {
                    ...
                    //cpu2
                    if ( m2.process1 == true ) {
                        if ( m2.nowtime-m2.cpubegin1 == cpu1[0].duration ) { // 該訂單處理完成

                            m2.logdata( "done", 2 ); // 記錄至donelist
                            cpu1.pop_back();
                            m2.process1 = false;
                            m2.newjob( 2 ); // 從佇列抓取新工作

                        }
                        else if ( m2.nowtime == cpu1[0].timeOut ) { // 該訂單逾時
                            m2.logdata( "abort", 2 ); // 記錄至adortlist
                            cpu1.pop_back();
                            m2.process1 = false;
                            m2.newjob( 2 ); // 從佇列抓取新工作
...
void mission2::newque(){
    while ( nowtime == data[whichdata].arrival && datadone == false ) {
        ...
        if ( process+quesize<=process1+quesize1 || cmd == "2" ){
            ...
            if ( process1 == false && quesize1==0 ) {//佇列及cpu皆沒東西
                cpu1.push_back( data[whichdata] );
                cpubegin1 = nowtime;
                process1 = true;
            }
            else if ( quesize1 < 3 ) {//佇列有空
