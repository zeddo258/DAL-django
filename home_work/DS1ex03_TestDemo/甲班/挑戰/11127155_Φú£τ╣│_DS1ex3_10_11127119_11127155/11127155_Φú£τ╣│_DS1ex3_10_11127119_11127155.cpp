...
class Cpu {
	...
	public:
		...
		bool processing() {  // CPU³B²zª¬ºA 
			return process;
		}
		
		int queueLength() {  // queueªø«× 
			return queue.length();
		}
		
		void enCpu(JobType job, int time) {  // §âjob©ñ¤JCPU
			cpu = job;
			process = true;
			completeTime = time + cpu.duration;
		}
		
		void enQueue(JobType job) {  // §âjob©ñ¤Jqueue 
			queue.enQueue(job);
		}
		
		void deQueue(int time) {  // §âjob±qqueue²¾¤JCPU 
			queue.deQueue(cpu);
			process = true;
			completeTime = time + cpu.duration;
		}
		
		State operation(JobType &job,int time) {  // ³B²zCPU¤ºªºjob 
...
class Simulation {
	...
	public:
		...
		bool openfile(string fileNumber) {  // ¶}±ÒÀÉ®× 
			return jobList.getAll(fileNumber);
		}
		void FIFOProcessing(){  // ³æ®Ö 
		...
		void FIFOWriteAllAsAFile(string fileNumber) {	// ³æ®Ö
			answers.putAll(fileNumber);
		}
		
		void SQFProcessing() {	// ¦h®Ö 
			JobType nextJob, cpu1Job, cpu2Job;
			State cpu1State, cpu2State;

			while(!jobList.isEmpty() || cpu1.processing() || cpu2.processing()) {	// ·íCPUÁÙ¦³job¦b³B²z©ÎjobList¤ºÁÙ¦³job
				if(cpu1.processing()) {  // (1)CPU³B²z 
					cpu1State = cpu1.operation(cpu1Job, time);
					if(cpu1State == DONE)	// CPU1³B²z§¹¦¨ 
						sqfAnswers.addDoneJob(cpu1Job.oid, 1, time, time - cpu1Job.duration - cpu1Job.arrival);
					
					else if(cpu1State == ABORT)  // CPU1³B²z¤¤¹O®É 
						sqfAnswers.addAbortJob(cpu1Job.oid, 1, time, time - cpu1Job.arrival); 
				}
				
				if(cpu2.processing()) {	   
					cpu2State = cpu2.operation(cpu2Job, time);
					if(cpu2State == DONE)	// CPU2³B²z§¹¦¨ 
						sqfAnswers.addDoneJob(cpu2Job.oid, 2, time, time - cpu2Job.duration - cpu2Job.arrival);
					
					else if(cpu2State == ABORT) // CPU2³B²z¤¤¹O®É 
						sqfAnswers.addAbortJob(cpu2Job.oid, 2, time, time - cpu2Job.arrival);
				}
			
				while(!cpu1.processing() && cpu1.queueLength() != 0 ) {  // (2)±q¦î¦C¨ú¥X 
					cpu1.deQueue(time);
					if(cpu1.operation(cpu1Job, time) == ABORT)	// ±q¦î¦C¤¤¨ú¥X©ñ¤JCPU1®É¹O®É 
						sqfAnswers.addAbortJob(cpu1Job.oid, 1, time, time - cpu1Job.arrival);
				}
				
				while(!cpu2.processing() && cpu2.queueLength() != 0 ) { 
					cpu2.deQueue(time);
					if(cpu2.operation(cpu2Job, time) == ABORT)	// ±q¦î¦C¤¤¨ú¥X©ñ¤JCPU2®É¹O®É 
						sqfAnswers.addAbortJob(cpu2Job.oid, 2, time, time - cpu2Job.arrival);
				}
				
				if(!jobList.isEmpty()){  // (3)jobList³B²z 
					jobList.nextJob(nextJob);
					while(nextJob.arrival == time && !jobList.isEmpty()) {
						if(!cpu1.processing()) {  // CPU1¨S¦³job¦b³B²z 
							cpu1.enCpu(nextJob, time);
						}
						
						else if(!cpu2.processing()) {  // CPU1³B²z¤¤¡BCPU2¨S¦³job¦b³B²z 
							cpu2.enCpu(nextJob, time);
						}
						
						else if(cpu1.queueLength() < 3 || cpu2.queueLength() < 3) {  // CPU1¡BCPU2ªº¦î¦C¤´¦³ªÅ¶¡ 
							if(cpu1.queueLength() <= cpu2.queueLength())	// CPU1ªº¦î¦CªÅ¶¡¬Ûµ¥©Î¤p©óCPU2 
								cpu1.enQueue(nextJob);
							
							else 	// CPU2ªº¦î¦CªÅ¶¡¤p©óCPU1 
								cpu2.enQueue(nextJob);
						}
						
						else	// CPU1¡BCPU2¦î¦C¬Òº¡ ->¨ú®ø 
		...
		void SQFWriteAllAsAFile(string fileNumber) {	// ³æ®Ö
...
void FIFOProcessing(string fileNumber) {  // ³æ®Ö³B²z 
...
void SQFProcessing(string fileNumber) {	 // ¦h®Ö³B²z 
	Simulation simulation;
    if(!simulation.openfile(fileNumber)) {
    	cout << "### sorted" << fileNumber << ".txt does not exist! ###" << endl;
    	return;
