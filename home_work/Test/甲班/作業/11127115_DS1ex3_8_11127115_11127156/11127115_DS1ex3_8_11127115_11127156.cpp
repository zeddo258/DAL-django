...
class AnsList {
  ...
  public:
    ...
    void addAbortJob( int OID, int abort, int delay );  // add item in abort list
    void addDoneJob( int OID, int departure, int delay );  // add item in done list
...
class Simulation {
  ...
  void updateQ();  // update each queue
  ...
  public:
    ...
    void outputFile( string fileName );  // write as a file
};  // class Simulation
  ...
  while ( stop == false ) {
    ...
    if ( command == "1") {
      ...
      if ( aList.getAll("sorted" + fileName + ".txt" ) ) {
        ...
        simulate.process();
        simulate.outputFile(fileName);
      }  // if

    }  // else if
    /*
    else if ( command == "3" ) {
      cout << "Input queue num:";
      cin >> numOfQueue;
    }  // else if
    */
    else if ( command == "0" ) {
...
// ========================== AnsList::addAbortJob() ==============================
void AnsList::addAbortJob( int OID, int abort, int delay ) {
  abortType temp;
  temp.OID = OID;
  temp.abort = abort;
  temp.delay = delay;
...
// =========================== AnsList::addDoneJob() ==============================
void AnsList::addDoneJob( int OID, int departure, int delay ) {
  doneType temp;
  temp.OID = OID;
...
void Simulation::process() {
  ...
  while ( !jobs.isEmpty() ) {
    ...
    // compare arrival time and current time
    if ( job.arrival >= allQueue[0] -> avail ) {

      while ( job.arrival >= allQueue[0] -> avail && isempty == false ) {
        // check queue is empty or not
        if ( allQueue[0] -> isEmpty() ) {
          // put in done list
          answers.addDoneJob( job.OID, job.arrival + job.duration, 0 );
      ...
      if ( job.arrival < allQueue[0] -> avail && isempty == false ) {
        // put in queue
        allQueue[0] -> enQueue( job );
        // cout << endl << job.arrival << " " << allQueue[0] -> avail << endl;
      }  // if

    }  // if

   // check there are queues have enough space to put job in
    else if (  SQF( index ) ) {
      allQueue[index] -> enQueue( job );
    }  // else if

    // 4.1: no space in queues
    else if ( job.arrival < allQueue[0] -> avail && !SQF( index ) ) {
      // put in abort list
      answers.addAbortJob( job.OID, job.arrival, 0 );
    }  // else if
    else {
...
void Simulation::updateQ() {
  JobType qFront;
  allQueue[0] -> getFront( qFront );

  // check there is any job in queue should be delete
  // 4.2: time out when job be taked out from queue
  while ( allQueue[0] -> avail >= qFront.timeout && !allQueue[0] -> isEmpty() ) {
    allQueue[0] -> deQueue();
    answers.addAbortJob( qFront.OID, allQueue[0] -> avail, allQueue[0] -> avail - qFront.arrival );
    if ( !allQueue[0] -> isEmpty() )
      allQueue[0] -> getFront( qFront );
  }  // while
  ...
  if ( !allQueue[0] -> isEmpty() ) {
    ...
    if ( allQueue[0] -> avail + qFront.duration > qFront.timeout ) {
      answers.addAbortJob( qFront.OID, qFront.timeout, qFront.timeout - qFront.arrival );
      allQueue[0] -> avail = qFront.timeout;
    }  // if

    // can finish work
    else {
      answers.addDoneJob( qFront.OID, allQueue[0] -> avail + qFront.duration, allQueue[0] -> avail - qFront.arrival );
      allQueue[0] -> avail = allQueue[0] -> avail + qFront.duration;
    }  // else
...
void Simulation::outputFile( string fileName ) {
