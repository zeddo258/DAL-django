...
class Queue {
    ...
    public :
        ...
        bool isEmpty() const { // return whether is empty
...
class JobList{
    private:
        ...
        } // showJob() // print information of a node
    public:
        bool load(string filename, float & timeload){ // ioad a file from input.txt
            Node temp1;
            bool isfinish = false;
            string temp;
            string s;
    ...
    void change( Node & sortfile, vector<jobType> & jtr ){ // Put the sorted result into sortfile
        Node temp, temp2;
        temp = file; // point temp to file
        bool found = false;
        for( int i = 0 ; i < jtr.size() ; i++ ){
                while( temp != NULL && found != true ){
                    if( temp -> oid == jtr[i].OID && temp -> arrival == jtr[i].arrival){
                        if( sortfile == NULL ){
                            sortfile = new node;
                            sortfile -> next = NULL;
                            temp2 = sortfile;
    ...
    void store( Node tempinputfile, Node & eventList ){ // store the last input data
        eventList = new node() ;
        eventList -> oid = tempinputfile -> oid ;
        eventList -> arrival = tempinputfile -> arrival ;
        eventList -> duration = tempinputfile -> duration ;
    ...
    void abortlist(Doneabort & abort, Node tempinputfile, int abortdepartures, int delays ){
        // record all the data in abort list in command 1
        Doneabort tail ;

        if( abort == NULL ) { // if the abort is empty
            abort = new doneabort() ;
    ...
    void abortlist(Doneabort & abort, Node tempinputfile, int CID, int abortdepartures, int delays ){
        // record all the data in abort list in command 2
        Doneabort tail ;
    ...
    void donelist(Doneabort & done, Node tempinputfile, int num, int num2 ){
        // record all the data in done list in command 1
        Doneabort tail, temp;

        if( done == NULL ) { // if done is empty
            done = new doneabort() ;
    ...
    void donelist(Doneabort & done, Node tempinputfile, int CID, int num, int num2 ){
        // record all the data in done list in command 2
        Doneabort tail, temp;
    ...
    void simulation1(Node & inputfile, Doneabort & done, Doneabort & abort ) {
            ...
            if( empty == true ) {
                if( bankQuene.length() != 0 ) {
                    while( find == false && bankQuene.length() != 0 ) {
                        ...
                        else if ( show -> timeout < time + show -> duration ){ // over time during still running
                            abortlist( abort, show, show -> timeout, show -> timeout - show -> arrival ) ;
    ...
    void simulation2(Node & inputfile, Doneabort & done, Doneabort & abort ) {
        Queue<Node> bankQuene ; // store queue
        Queue<Node> bankQuene2 ; // store queue2
        Node temp, show ;
        temp = new node() ;
        Node tempinputfile = inputfile ;

        Node eventList = NULL ; // store GPU
        Node eventList2 = NULL ; // store GPU2
        done = NULL ;
        abort = NULL ;
        bool end = false ;
        ...
        count = tempinputfile -> arrival + tempinputfile -> duration;
        donelist( done, tempinputfile , 1, count, 0 ) ; // store first data in done
        tempinputfile = tempinputfile -> next ;

        count2 = tempinputfile -> arrival + tempinputfile ->duration ;
        donelist( done, tempinputfile , 2, count2, 0 ) ; // store second data in done
        tempinputfile = tempinputfile -> next ;

        for ( tempinputfile; tempinputfile != NULL; tempinputfile = tempinputfile -> next ){
            bool empty = false ;
            bool empty2 = false ;

            if( tempinputfile -> arrival >= count ) { // the first queue
                while( tempinputfile -> arrival >= count && bankQuene.length() != 0 ) {
                    bankQuene.dequeue(temp) ;
                    if ( temp -> duration + count == temp ->timeout ) { // done situation
                        donelist( done, temp , 1, count+temp -> duration, ( count - temp -> arrival ) ) ;
                        count = temp -> timeout ;
                    }
                    else if ( temp -> duration + count < temp ->timeout ) { // done situation
                        donelist( done, temp , 1, count+temp -> duration, ( count - temp -> arrival ) ) ;
                        count = temp -> duration + count ;
                    }
                    else if ( count < tempinputfile -> timeout ){ // abort situation
                        if ( temp -> timeout < tempinputfile -> arrival && ( count > temp ->timeout ) ) abortlist( abort, temp, 1, count, ( count - temp -> arrival ) ) ;
                        else abortlist( abort, temp, 1, temp ->timeout, ( temp ->timeout - temp -> arrival ) ) ; // still running but overtime
                        if( count < temp -> timeout ) count = temp -> timeout ;
                    }
                }
            }
            if( tempinputfile -> arrival >= count2 ) { // the second queue
                while( tempinputfile -> arrival >= count2 && bankQuene2.length() != 0 ) {
                    bankQuene2.dequeue(temp) ;
                    if ( temp -> duration + count2 == temp ->timeout ) { // done situation
                        donelist( done, temp , 2, count2+temp -> duration, ( count2 - temp -> arrival ) ) ;
                        count2 = temp -> timeout ;
                    }
                    else if ( temp -> duration + count2 < temp ->timeout ) { // done situation
                        donelist( done, temp , 2, count2+temp -> duration, ( count2 - temp -> arrival ) ) ;
                        count2 = temp -> duration + count2 ;
                    }
                    else if ( count2 < tempinputfile -> timeout ){ // abort situation
                        if ( temp ->timeout < tempinputfile -> arrival && ( count2 > temp ->timeout ) ) abortlist( abort, temp, 2, count2, ( count2 - temp -> arrival ) ) ;
                        else abortlist ( abort, temp, 2, temp ->timeout, ( temp ->timeout - temp -> arrival ) ) ; // still running but overtime
                        if( count2 < temp -> timeout ) count2 = temp -> timeout ;
                    }
                }
            }

            if( bankQuene.length() == 0 && count <= tempinputfile -> arrival ){ // no data in queue
                count = tempinputfile -> arrival + tempinputfile ->duration ; // put one in CPU
                donelist( done, tempinputfile , 1, count, 0 ) ;
            }
            else if ( bankQuene2.length() == 0 && count2 <= tempinputfile -> arrival ){ // no data in queue
                count2 = tempinputfile -> arrival + tempinputfile ->duration; // put one in CPU
                donelist( done, tempinputfile , 2, count2, 0 ) ;
            }
            else if( tempinputfile -> arrival <= count || tempinputfile -> arrival <= count2 ) { // put data in queue
                if( bankQuene.length() >= 3 && bankQuene2.length() >= 3) abortlist( abort, tempinputfile, 0, tempinputfile -> arrival, 0 ) ;
                else if ( bankQuene.length() <= bankQuene2.length() ) bankQuene.enqueue( tempinputfile ) ;
                else bankQuene2.enqueue( tempinputfile ) ;
            }

            if( tempinputfile -> next == NULL ) store( tempinputfile, eventList) ;
        } // for


            while( bankQuene.length() != 0 ) { // deal with the data in queue1 and same as the way before
                bankQuene.dequeue(temp) ;
                if ( temp -> duration + count == temp ->timeout ) { // done situation
                    donelist( done, temp , 1, count+temp -> duration, ( count - temp -> arrival ) ) ;
                    count = temp -> timeout ;
                    ...
                    if ( temp ->timeout < eventList -> arrival && ( count > temp ->timeout ) ) abortlist( abort, temp, 1, count, ( count - temp -> arrival ) ) ;
                    else abortlist( abort, temp, 1, temp ->timeout, ( temp ->timeout - temp -> arrival ) ) ; // still running but overtime
                    if( count < temp -> timeout ) count = temp -> timeout ;
                }
            }

            while( bankQuene2.length() != 0 ) { // deal with the data in queue2 and same as the way before
                bankQuene2.dequeue(temp) ;
                if ( temp -> duration + count2 == temp ->timeout ) { // done situation
                    donelist( done, temp , 2, count2+temp -> duration, ( count2 - temp -> arrival ) ) ;
                    count2 = temp -> timeout ;
                    ...
                    if ( count2 > temp ->timeout ) abortlist( abort, temp, 2, count2, ( count2 - temp -> arrival ) ) ;
                    else abortlist( abort, temp, 2, temp ->timeout, ( temp ->timeout - temp -> arrival ) ) ; // still running but overtime
                    if( count2 < temp -> timeout ) count2 = temp -> timeout ;
                }
            }
    ...
    void cccount(Doneabort & abort,Doneabort & done, float & averagedelay, float & successrate){ // count average delay and success rate
        ...
        for( i = 0; temp != NULL; temp = temp -> next, i++ ){
            num = num + temp->delay ;
        } // count the all delay in queue1

        for( j = 0; temp1 != NULL; temp1 = temp1 -> next, j++ ){
            sum = sum + temp1->delay ;
        } // count the all delay in queue2
    ...
    void outputsimulation(Doneabort & done, Doneabort & abort,string num, float averagedelay, float successrate ){ // create simulation output.txt
        ...
        string t = ""; // clear the stream
        stringstream tem;
        tem.precision(2); //set the float of two words
        tem.setf(std::ios::fixed);
        tem << averagedelay;
        t = tem.str();
        sortedfile << "[Average Delay]\t"<< t << " ms\n";
        tem.str("");// clear the stringstream
        tem.precision(2);
        tem.setf(std::ios::fixed);
        tem << successrate;
        t = tem.str();
    ...
    void outputdoblesimulation(Doneabort & done, Doneabort & abort,string num, float averagedelay, float successrate ){ // create simulation output.txt
...
int main(){
    ...
    do {
            ...
            if( command == 1 || command == 2 || command == 3 )
            {
                ...
                if( command == 1){
                    if(inputfile.load(fileName,timeread)){ // success open file
                        file = inputfile.GetFile();
                        jt = inputfile.GetVector(); // only have oid and arrival
                        inputfile.Shellsort(jt,timesort);
                        inputfile.change(sortfile, jt);
                        inputfile.outputfile(sortfile,num,timewrite); // show ouput.txt
                        inputfile.showsort(file);
                        cout << endl<<"Reading data: "<< (int)timeread << " clocks" << "("
                              << fixed<<setprecision(2) << timeread <<"ms)."<< endl; // show record time of reading a file
                        cout << "Sorting data: "<< (int)timesort << " clocks" << "("
                              << fixed<<setprecision(2) << timesort <<"ms)."<< endl;// show record time of sorting a file
                        cout << "Writing data: "<< (int)timewrite << " clocks" << "("
                              << fixed<<setprecision(2) << timewrite <<"ms)."<< endl; // show a record time of output a file
                    ...
                    if(inputfile.loadsorted(fileName)){
                        file = inputfile.GetFile();
                        if(command == 2 )inputfile.simulation1(file, done, abort); // 1 CPU simulation
                        else inputfile.simulation2(file,done,abort); // 2 CPU simulation
                        inputfile.cccount(abort,done,averagedelay,successrate); // calculate the result of avergedelay and successrate
                        if( command == 2 )inputfile.outputsimulation(done,abort,num,averagedelay,successrate); // 1CPU output
                        else inputfile.outputdoblesimulation(done,abort,num,averagedelay,successrate); // 2 CPU output
                        cout << endl << "The simulation is running...\n";
                        if(command == 2) cout << "See output" << num << ".txt\n";
                        else cout << "See double" << num << ".txt\n";
                    }
