...
class JobList {
	...
	public:
		...
		bool taskthree ( string fileName );
};
class runtaskthree{
		vector<done> dojob; //完成清單的vector 
		vector<jobType> aListt;
		...
		vector<jobType> decide;
	public:
		void dotaskthree( vector<jobType> aList , string fileName);
};
void runtaskthree::dotaskthree( vector<jobType> aList , string fileName ){
	int nor = 0;
	int dojobs = 0;
	Queue aQueue;
	Queue aQueue2;
	...
	ifs.close();
	int cpu1 = aListt.at(0).arrival ; // cpu 開始時間為第一筆資料的arrival
	int oid1 = aListt.at(0).OID;
	int cpu2 = aListt.at(1).arrival ; // cpu 開始時間為第一筆資料的arrival
	int oid2 = aListt.at(1).OID;
	int time = 0, time2 = 0;
	bool take1 = false;
	bool take2 = false;
	bool reuse1 = false;
	...
	for ( int i =0; i < aListt.size() ;i++){
		take1 = false;
		take2 = false;
		if ( i>1 && ! reuse1 && ! reuse2){
			if ( cpu1 <= aListt.at(i).arrival && ! aQueue.isEmpty() && cpu2 > aListt.at(i).arrival ){
				take1 = true;
			}
			else if ( cpu2 <= aListt.at(i).arrival && ! aQueue2.isEmpty() &&  cpu1 > aListt.at(i).arrival ){
				take2 = true;
			}
			
			else if( cpu1 <= aListt.at(i).arrival && ! aQueue.isEmpty() && cpu2 <= aListt.at(i).arrival && ! aQueue2.isEmpty()  ){
				take1 = true;
			}
			else if ( cpu1 > aListt.at(i).arrival && cpu2 > aListt.at(i).arrival && (time!=3 || time2!=3)){
				if ( time < time2 ) take1 = true;
				else if ( time2 < time ) take2 = true;
				else if ( time == time2 ) take1 = true;
			}
			else if ( cpu1 > aListt.at(i).arrival && cpu2 > aListt.at(i).arrival && time==3 && time2==3 ){
				cannotdo.push_back( abt );
				cannotdo.at(nor).OID = aListt.at(i).OID;
				cannotdo.at(nor).CID = 0;
				cannotdo.at(nor).abort = aListt.at(i).arrival;
				cannotdo.at(nor).delay = 0;
				nor++;
			}
		}
		if ( take1 == true || i == 0 || reuse1 == true ){
			reuse1 = false;
			if ( aListt.at(i).duration + cpu1 <= aListt.at(i).timeout && aListt.at(i).arrival >=cpu1   && aQueue.isEmpty() ) {
				dojob.push_back( D );
				dojob.at(dojobs).OID = aListt.at(i).OID;
				dojob.at(dojobs).CID = 1;
				dojob.at(dojobs).departure = cpu1 + aListt.at(i).duration;
				dojob.at(dojobs).delay = cpu1 - aListt.at(i).arrival;
				cpu1 = cpu1 + aListt.at(i).duration;
				dojobs++;
			}
			
			else if (   aListt.at(i).arrival > cpu1 ){ //閒置
				decide.push_back(type);
				decide.at(0) = aQueue.dequeue();
				if ( cpu1 < decide.at(0).arrival ) cpu1 = decide.at(0).arrival;
				if ( cpu1 + decide.at(0).duration > decide.at(0).timeout){ // 若cpu + duration > timeout 則放到取消清單中 
					if ( cpu1 > decide.at(0).timeout ){ //若 cpu > timeout 
						cannotdo.push_back( abt );
						cannotdo.at(nor).OID = decide.at(0).OID;
						cannotdo.at(nor).CID = 1;
						cannotdo.at(nor).abort = cpu1;
						...
						cannotdo.at(nor).delay = cannotdo.at(nor).abort - decide.at(0).arrival;
						nor++;
						cpu1 = decide.at(0).timeout; // 改變 cpu時間為timeout 
					}
				}
				else if ( cpu1 + decide.at(0).duration <=  decide.at(0).timeout ){ // cpu + duration <= timeout 則放到完成清單中 
					dojob.push_back( D );
					dojob.at(dojobs).OID = decide.at(0).OID;
					dojob.at(dojobs).CID = 1;
					dojob.at(dojobs).departure = cpu1 + decide.at(0).duration;
					dojob.at(dojobs).delay = cpu1 -  decide.at(0).arrival;
					cpu1 = cpu1 + decide.at(0).duration;
					dojobs++;
				}
				time--;
			...
			else if (  aListt.at(i).arrival == cpu1 ) { // 如果這筆資料會的arrival等於cpu, 則先清空佇列內的資料 
		...
		else if ( take2 == true || i == 1 || reuse2 == true ){
			reuse2 = false;
			if (  aListt.at(i).duration + cpu2 <= aListt.at(i).timeout && aListt.at(i).arrival >=cpu2   && aQueue2.isEmpty() ) {
				dojob.push_back( D );
				dojob.at(dojobs).OID = aListt.at(i).OID;
				dojob.at(dojobs).CID = 2;
				dojob.at(dojobs).departure = cpu2 + aListt.at(i).duration;
				dojob.at(dojobs).delay = cpu2 - aListt.at(i).arrival;
				cpu2 = cpu2 + aListt.at(i).duration;
				dojobs++;
			}
			else if ( aListt.at(i).arrival > cpu2 ){ //閒置
				if ( ! aQueue2.isEmpty() ){
					decide.push_back(type);
					decide.at(0) = aQueue2.dequeue();
					if ( cpu2 < decide.at(0).arrival ) cpu2 = decide.at(0).arrival;
					if ( cpu2 + decide.at(0).duration > decide.at(0).timeout){ // 若cpu + duration > timeout 則放到取消清單中 
						if ( cpu2 > decide.at(0).timeout ){ //若 cpu > timeout 
							cannotdo.push_back( abt );
							cannotdo.at(nor).OID = decide.at(0).OID;
							cannotdo.at(nor).CID = 2;
							cannotdo.at(nor).abort = cpu2;
							cannotdo.at(nor).delay = cannotdo.at(nor).abort - decide.at(0).arrival;
							nor++;
						}
						else { // 若 cpu <= timeout   
							...
							cannotdo.at(nor).delay = cannotdo.at(nor).abort - decide.at(0).arrival;
							nor++;
							cpu2 = decide.at(0).timeout; // 改變 cpu時間為timeout 
						}
					}
				
				
					else if ( cpu2 + decide.at(0).duration <=  decide.at(0).timeout ){ // cpu + duration <= timeout 則放到完成清單中 
						dojob.push_back( D );
						dojob.at(dojobs).OID = decide.at(0).OID;
						dojob.at(dojobs).CID = 2;
						dojob.at(dojobs).departure = cpu2 + decide.at(0).duration;
						dojob.at(dojobs).delay = cpu2 -  decide.at(0).arrival;
						cpu2 = cpu2 + decide.at(0).duration;
						dojobs++;
					}
					time2--;
				}
				aQueue2.enqueue( aListt.at(i) );
				time2++;
			}
			else if (  aListt.at(i).arrival == cpu2 ) { // 如果這筆資料會的arrival等於cpu, 則先清空佇列內的資料 
				if ( ! aQueue2.isEmpty() ){ //代表佇列不為空 
					for( int j = 0; ! aQueue2.isEmpty() ; j++ ){
						decide.push_back(type);
						decide.at(0) = aQueue2.dequeue(); // 把佇列的第一個丟出來 
						if ( cpu2 + decide.at(0).duration > decide.at(0).timeout){ // 若cpu + duration > timeout 則放到取消清單中  
							if ( cpu2 > decide.at(0).timeout ){ //若 cpu > timeout  
								cannotdo.push_back( abt );
								cannotdo.at(nor).OID = decide.at(0).OID;
				...
				if ( time2 < 3 ) { //若佇列中小於三個人 
					aQueue2.enqueue( aListt.at(i) ); // 存入陣列中 
					time2++;
				}
				else {  // 若還要存入佇列,則直接存到取消清單中 
					cannotdo.push_back( abt );
...
bool JobList::taskthree( string fileName ){
	runtaskthree start;
	start.dotaskthree( aList, fileName );
