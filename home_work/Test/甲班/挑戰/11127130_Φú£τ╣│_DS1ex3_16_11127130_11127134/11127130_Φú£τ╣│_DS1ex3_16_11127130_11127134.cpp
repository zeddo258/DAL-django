...
void storeDouble(string fileNumber, vector<vector <int> > abortProcess, vector<vector <int> > doneProcess,
				 double averageDelay, double successRate);                      // 建double檔  
...
void doQueuesFront(ProcessQueue aQueue[], vector<vector <int> > &abortProcess,
				   vector<vector <int> > &doneProcess, int index);              // 執行佇列第一項(多佇列) 
...
int main() {
    ...
    while (true) {
		...
		if (command == 1) {
			...
			if (success){
				cout << endl << "The simulation is running...";				
				ProcessQueue aQueue[2];                                         // 佇列 
				vector<vector <int> > abortProcess;                             // 取消清單 
				vector<vector <int> > doneProcess;                              // 完成清單 
				bool processDone;
				bool avail[2];                                                  // cpu是否空閒 
				int cpuNum;                                                     // cpu編號-1(佇列陣列的index) 
				...
				for (int i = 0; i < process.size(); i++) {
					...
					int newAvailTime[2], queIndex[2];                           // 預測cpu空閒時間, 佇列index 
					// 判斷哪個cpu是空閒的 
					for (int j = 0; j < 2; j++) { 
						queIndex[j] = 0;
						...
						while (process[i][1] >= newAvailTime[j] && queIndex[j] < aQueue[j].length()) {
							vector <int> temp;
							aQueue[j].get(queIndex[j], temp);                   // 取得第queIndex[j] + 1項工作的資訊 
							// temp[0]:工作編號, temp[1]:進入時間, temp[2]:持續時間, temp[3]:逾時時間 
							if (newAvailTime[j] + temp[2] > temp[3]) {          // 逾時 
								if (newAvailTime[j] < temp[3]) {                // 工作執行途中會逾時 
									newAvailTime[j] = temp[3];                  // 預測cpu空閒時間設為該工作的逾時時間 
								}								
							}
							else {                                              // 不會逾時  
								newAvailTime[j] = newAvailTime[j] + temp[2];    // 預測cpu空閒時間設為該工作完成時間  
							}
							
							queIndex[j]++;	                                    // 檢查佇列中下一筆工作  
						}
					
						if (newAvailTime[j] <= process[i][1]) {                 // 工作進來時cpu是空閒的 
							avail[j] = true;
						}
					}
					
					if (avail[0]) {                                             // 一號cpu空閒或兩個cpu都空閒 
						cpuNum = 0;
					}
					else if (avail[1]) {                                        // 只有二號cpu空閒 
						cpuNum = 1;
					}
					else {                                                      // 都沒閒置 
						if (aQueue[0].length() - queIndex[0] == aQueue[0].max() && aQueue[1].length() - queIndex[1] == aQueue[1].max()) { // 佇列都滿 
							cpuNum = -1;                                        // 直接取消 
						}
						else if (aQueue[0].length() - queIndex[0] <= aQueue[1].length() - queIndex[1]) { // 一號佇列較短 
							cpuNum = 0;
						}
						else {                                                  // 二號佇列較短 
							cpuNum = 1; 
					...
					if (cpuNum != -1) {                                         // 沒有被直接取消 
						// 比較兩佇列長度，先更新較長的佇列，再更新較短的佇列；若長度一樣，則先更新一號佇列 
						if (aQueue[1 - cpuNum].length() - queIndex[1 - cpuNum] > aQueue[cpuNum].length() - queIndex[cpuNum]) { 
							...
							while (process[i][1] >= aQueue[cpuNum].availTime) {
								if (!aQueue[cpuNum].isEmpty()) {							
									...
									// 直接執行並完成該工作，加入完成清單 
									aQueue[cpuNum].availTime = process[i][1] + process[i][2]; // CPU空閒時間設為該工作完成時間 
									processDone = true;	
									vector <int> done;
									done.push_back(process[i][0]);
									done.push_back(cpuNum + 1); 
									done.push_back(aQueue[cpuNum].availTime);   // 完成時間  
									done.push_back(0);                          // 無延遲 
									doneProcess.push_back(done);						
								}
							}
							...
							if (!queueIsEmpty && cpuNum == 0) {
								aQueue[0].enQueue(process[i]); // 排進佇列 
							}
							...
							if (!queueIsEmpty && cpuNum == 1) {                                            
								aQueue[1].enQueue(process[i]); // 排進佇列 
							}
							
							// 若選到的佇列空了，則該工作加入完成清單 
							if (queueIsEmpty && cpuNum == 0) { // 選到第一個佇列的情況 
								aQueue[0].availTime = process[i][1] + process[i][2]; // CPU空閒時間設為該工作完成時間 	
								vector <int> done;
								done.push_back(process[i][0]);
								done.push_back(1);
								done.push_back(aQueue[0].availTime);                 // 完成時間  
								done.push_back(0);                                   // 無延遲 
								doneProcess.push_back(done);								
							}	
							
							// 若選到的佇列空了，則該工作加入完成清單 
							if (queueIsEmpty && cpuNum == 1) { // 選到第二個佇列的情況 
								aQueue[1].availTime = process[i][1] + process[i][2]; // CPU空閒時間設為該工作完成時間 
								vector <int> done;
								done.push_back(process[i][0]);
								done.push_back(2);
								done.push_back(aQueue[1].availTime);                 // 完成時間  
								done.push_back(0);                                   // 無延遲 
								doneProcess.push_back(done);								
							}						 
						}
				...
				storeDouble(fileNumber, abortProcess, doneProcess, averageDelay, successRate); // 建立double檔 
...
void doQueuesFront(ProcessQueue aQueue[], vector<vector <int> > &abortProcess,
				   vector<vector <int> > &doneProcess, int index) { // 執行佇列第一項工作(多佇列) 
	vector <int> temp;
	aQueue[index].getFront(temp);                         // 取得第一項工作的資訊 
	// temp[0]:工作編號, temp[1]:進入時間, temp[2]:持續時間, temp[3]:逾時時間 
	if (aQueue[index].availTime + temp[2] > temp[3]) {    // 逾時 
		if (aQueue[index].availTime < temp[3]) {          // 執行途中發現逾時 
			// 加入取消清單 
			aQueue[index].availTime = temp[3];            // CPU空閒時間設為該工作的逾時時間 
			vector <int> abort;
			abort.push_back(temp[0]);
			...
			abort.push_back(temp[0]);
			abort.push_back(index + 1);
			abort.push_back(aQueue[index].availTime);     // 取消時間 
			abort.push_back(aQueue[index].availTime - temp[1]);  // 延遲為取消時間減進入時間  
			abortProcess.push_back(abort);							
		}					
	}
	else {                                                // 沒有逾時  
		...
		done.push_back(aQueue[index].availTime + temp[2]); // 完成時間為CPU空閒時間加工作持續時間 
		done.push_back(aQueue[index].availTime - temp[1]); // 延遲為開始執行該工作的時間減進入時間 
		aQueue[index].availTime = aQueue[index].availTime + temp[2]; // CPU空閒時間設為該工作完成時間 									
		doneProcess.push_back(done); 
	}
	
	aQueue[index].deQueue();	                          // 將工作從佇列中移除 
}
